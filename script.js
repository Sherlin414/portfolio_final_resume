/* =========================================================
   ...
   localStorage-based. DATA_VERSION forces a fresh load when
   default data changes (e.g. project links updated).
   ========================================================= */

const STORAGE_KEY = "portfolio:sherlin_24104082";
const DATA_VERSION = 3; // bump this whenever defaults change
...
const DEFAULT_INTERNSHIPS = [
  {
    ...
    link: "https://www.linkedin.com/posts/sherlins_webdevelopment-internshipcompletion-prodigyinfotech-activity-7370393129771986944-41Rv?utm_source=share&utm_medium=member_desktop&rcm=ACoAAFGCnpMB2cjb1083m29Jlcx4934GOX6BPNc"
  },
  {
    id: "blk-intern-2",
    ...
    link: "https://www.linkedin.com/posts/sherlins_internship-fullstackdevelopment-generativeai-share-7475468511482531840-QOFG/?utm_source=share&utm_medium=member_desktop&rcm=ACoAAFGCnpMB2cjb1083m29Jlcx4934GOX6BPNc"
  },
  {
    id: "blk-intern-3",
    ...
    link: "https://drive.google.com/drive/folders/1YnjWXFUoJITrQCzpgdij9sA06a-06oQd?usp=drive_link"
  },
];
...
const DEFAULT_PROJECTS = [
  {
    ...
    tags: ["HTML", "CSS", "JavaScript"],
    link: "https://69ca946955c588083079a980--jazzy-tanuki-aad622.netlify.app/"
  },
  {
    id: "blk-proj-2",
    ...
    tags: ["HTML", "CSS", "JavaScript"],
    link: "https://ai-mafia-game-web-de-iw5i.bolt.host"
  },
  {
    id: "blk-proj-3",
  ...
  },
  {
    ...
    tags: ["HTML", "CSS", "JavaScript"],
    link: "https://sherlin414.github.io/stopwatch/"
  },
  {
    id: "blk-proj-6",
    title: "Tic Tac Toe",
    ...
    tags: ["HTML", "CSS", "JavaScript"],
    link: "https://sherlin414.github.io/Tic-tac-toe/"
  },
  {
    id: "blk-proj-7",
    title: "Weather App",
    ...
    tags: ["HTML", "CSS", "JavaScript"],
    link: "https://sherlin414.github.io/Weather/"
  },
  {
    id: "blk-proj-8",
    ...
    tags: ["HTML", "CSS", "JavaScript"],
    link: "https://sherlin414.github.io/Landing-page-/"
  },
  {
    id: "blk-proj-9",
  ...
  },
  {
    ...
    link: "https://drive.google.com/drive/folders/1MisKWMfvJMTfQ_SJBEc0tW-yLgwL1zyp?usp=drive_link"
  },
  {
    id: "blk-proj-11",
    title: "Gravity Maze Goal",
    desc: "Gravity Maze is an interactive web-based puzzle game. Players control gravity to navigate a maze, overcome obstacles, and reach the goal. Focuses on logic, timing, and problem-solving.",
    tags: ["HTML", "CSS", "JavaScript"],
    link: "https://sherlin414.github.io/gravity_maze_goal/"
  },
  {
    id: "blk-proj-12",
    title: "Neon Path Zip",
    desc: "Neon Path Zip is an interactive web-based puzzle game. Players navigate through neon paths, connect routes, and complete challenges using logical thinking and strategy.",
    tags: ["HTML", "CSS", "JavaScript"],
    link: "https://neon-path-zip.netlify.app/"
  },
  {
    id: "blk-proj-13",
    title: "AI-Powered College Attendance Management System",
    desc: "An intelligent system that monitors student attendance patterns using AI. Automatically sends alert notifications to students whose attendance falls below 70% and delivers daily motivational messages to students with 100% attendance.",
    tags: ["HTML", "CSS", "JavaScript", "Google Colab", "Python"],
    link: "https://github.com/Sherlin414/AI-Powered-College-Attendance-Management-System.git"
  }
];
...
function updateFooterHeight() {
  ...
  if (!el) return;
  el.textContent = 8 + state.skills.length + state.projects.length +
    state.internships.length + state.certificates.length;
}
...
async function loadData() {
  try {
    ...
    if (raw) {
      ...
      // If saved version is older than current, wipe and reload defaults
    ...
    console.log("No saved data, using defaults:", e);
  }
  return null;
}
...
async function init() {
  ...
  if (data) {
    ...
    state.skills     = data.skills     || DEFAULT_SKILLS.map(s => ({ ...s, hash: hashId() }));
    state.projects   = data.projects   || DEFAULT_PROJECTS.map(p => ({ ...p, hash: hashId() }));
...
function renderSkills() {
  ...
  skillsEmpty.classList.toggle("hidden", state.skills.length > 0);
...
function openSkillModal(skill) {
  ...
  `, async () => {
    ...
    if (isEdit) { skill.name = name; skill.level = level; }
    else state.skills.push({ id: genId("skill"), name, level, hash: hashId() });
    renderSkills();
    await saveData();
...
function openProjectModal(proj) {
  ...
  `, async () => {
    ...
    if (isEdit) { proj.title = title; proj.desc = desc; proj.tags = tags; proj.link = link; }
    else state.projects.push({ id: genId("proj"), title, desc, tags, link, hash: hashId() });
    renderProjects();
    await saveData();
...
function renderInternships() {
  ...
  internshipsEmpty.classList.toggle("hidden", state.internships.length > 0);
...
function openInternshipModal(intern) {
  ...
  `, async () => {
    ...
    if (isEdit) { intern.title = title; intern.org = org; intern.mode = mode; intern.desc = desc; intern.tags = tags; intern.link = link; }
    else state.internships.push({ id: genId("intern"), title, org, mode, desc, tags, link, hash: hashId() });
    renderInternships();
    await saveData();
...
// Certificate images: save your cert photos named as per imgFile field
// in the same folder as index.html  e.g. cert_iot.jpg, cert_python.jpg ...
// If the file doesn't exist the card shows a styled placeholder instead.
function certImageHTML(cert) {
  if (!cert.imgFile) return "";
  return `
    <div class="cert-img-wrap">
      <img
        src="${escapeHTML(cert.imgFile)}"
...
function renderCertificates() {
  ...
  certificatesEmpty.classList.toggle("hidden", state.certificates.length > 0);
...
function openCertificateModal(cert) {
  ...
  openModal(isEdit ? "Update certificate block" : "Add certificate block", `
    ...
    <div>
      <label for="m-cert-meta">Details (comma separated)</label>
      <input type="text" id="m-cert-meta" value="${cert ? escapeHTML((cert.meta || []).join(", ")) : ""}" placeholder="e.g. 85% score, Jul–Oct 2025, 12-week course">
    </div>
    <div>
      <label for="m-cert-img">Image filename (e.g. cert_iot.jpg)</label>
      <input type="text" id="m-cert-img" value="${cert ? escapeHTML(cert.imgFile || "") : ""}" placeholder="Place the image file in same folder as index.html">
    </div>
    <button type="submit" class="btn btn-primary btn-block">${isEdit ? "Save changes" : "Add block"}</button>
  `, async () => {
    ...
    const imgFile = document.getElementById("m-cert-img").value.trim();
    if (!name || !issuer) return false;
    if (isEdit) { cert.name = name; cert.issuer = issuer; cert.badge = badge; cert.meta = meta; cert.imgFile = imgFile; }
    else state.certificates.push({ id: genId("cert"), name, issuer, badge, meta, imgFile, hash: hashId() });
    renderCertificates();
    await saveData();
...
function renderMessages() {
  ...
  messagesEmpty.classList.toggle("hidden", state.messages.length > 0);
